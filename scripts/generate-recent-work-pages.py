...
def photo_src(filename: str) -> str:
    return f"../images/recent-work/{filename}"


def project_photos(project: dict) -> list[dict]:
    return list(project.get("photos") or [])


def gallery_html(project: dict) -> str:
    photos = project_photos(project)
    slides = [
        (
            '<div class="job-hero-placeholder" data-photo-fallback="1">'
            f'<p class="service-badge">{project["categoryLabel"]}</p>'
            f'<p class="text-white text-xl font-semibold mt-3">{project["location"]}</p>'
            "</div>"
        )
    ]
    for photo in photos:
        src = photo_src(photo["file"])
        alt = photo["alt"]
        slides.append(
            f'<img src="{src}" alt="{alt}" width="1200" height="800" '
            f'loading="lazy" onclick="openLightbox(\'{src}\')" '
            'onerror="this.remove();" style="cursor:pointer">'
        )
    return '<div class="job-gallery">' + "\n".join(slides) + "</div>"


def page_html(project: dict) -> str:
    url = f"https://scwellservice.com/recent-work/{project['slug']}.html"
    photos = project_photos(project)
    og_image = (
        f"https://scwellservice.com/images/recent-work/{photos[0]['file']}"
        if photos
...
{json.dumps({
    ...
    "publisher": {
        ...
        "logo": "https://scwellservice.com/images/logo-text-only-3x.png"
    },
...
<style>
        ...
        @media (min-width: 768px) {{
            ...
            .job-gallery img:first-child {{ grid-column: 1 / -1; }}
        }}
        .job-gallery img {{
            width: 100%;
            height: auto;
...
<a class="text-white font-semibold whitespace-nowrap" href="./">Recent Work</a>
...
<a class="block text-white font-semibold py-2" href="./">Recent Work</a>
...
<div class="max-w-3xl mx-auto px-4 py-3 text-sm text-gray-500">
<a class="hover:text-accent" href="../">Home</a> › <a class="hover:text-accent" href="./">Recent Work</a> › <span class="text-gray-700">{project['title']}</span>
</div>
</div>
...
<h2 class="text-3xl font-bold mb-4">Need Well Service?</h2>
<p class="text-lg opacity-90 mb-8">From emergency repairs to new well drilling, our team is ready to help.</p>
<div class="flex flex-col sm:flex-row gap-4 justify-center">
<a class="bg-white text-primary font-bold px-8 py-3 rounded-full hover:bg-gray-100 transition text-lg" href="tel:7604408520">📞 (760) 440-8520</a>
<a class="bg-accent text-white font-bold px-8 py-3 rounded-full hover:bg-accent/90 transition text-lg" href="sms:6192590410" title="Text-only line: (619) 259-0410">💬 Text Us</a>
<a class="border-2 border-white text-white font-bold px-8 py-3 rounded-full hover:bg-white hover:text-primary transition text-lg" href="../cost-calculator.html">Free Estimate</a>
</div>
</div>
</section>
...
<a class="flex-1 bg-blue-500 text-white text-center py-2.5 rounded-full font-bold text-sm" href="sms:6192590410" title="Text-only line: (619) 259-0410">💬 Text Us</a>
...
<script>
    ...
    document.querySelectorAll('.job-gallery').forEach(function (gallery) {{
        var fallback = gallery.querySelector('[data-photo-fallback]');
        function sync() {{
            var any = Array.prototype.some.call(gallery.querySelectorAll('img'), function (img) {{
                return img.complete && img.naturalWidth > 0;
            }});
            if (fallback) fallback.style.display = any ? 'none' : '';
        }}
        gallery.querySelectorAll('img').forEach(function (img) {{
            img.addEventListener('load', sync);
            img.addEventListener('error', function () {{ img.remove(); sync(); }});
        }});
        sync();
    }});
</script>
...
def main() -> None:
    ...
    for project in PROJECTS:
        ...
        path.write_text(page_html(project))
        print(f"wrote {path.relative_to(ROOT)} ({len(project_photos(project))} photo refs)")

#!/usr/bin/env node
/**
 * Update services city pages with the full header from main site
 * These pages use inline CSS, so we need to add Tailwind + the header
 */
...
// New header HTML (paths use absolute paths since services/ is at root level)
const newHeader = `
    <!-- Emergency Banner -->
        ...
        if (entry.isDirectory()) {
            ...
            try {
                ...
                if (content.includes('<header class="header">')) {
                    // Add Tailwind before </head> if not present
                    ...
                    console.log(`? Unknown format: ${fullPath}`);
                    skipped++;
                }
            } catch (err) {
...
console.log('Updating services city pages...\n');

#!/usr/bin/env python3
"""Replace ALL blog headers with EXACT copy of homepage header (paths adjusted)"""
...
# EXACT homepage header with paths adjusted for /blog/ subdirectory
...
for filepath in BLOG_DIR.glob("*.html"):
    ...
    try:
        ...
        # Find everything from <body to </header> and the script after
        # Replace it all with our template
        pattern = r'<body class="bg-white">.*?</header>(\s*<script>.*?</script>)?'
        ...
        if re.search(pattern, content, re.DOTALL):
            ...
            errors += 1
            print(f"Pattern not found: {filepath.name}")
    except Exception as e:
        errors += 1
        print(f"Error {filepath.name}: {e}")

print(f"✓ Replaced headers in {updated} blog posts")
if errors:
    print(f"⚠ {errors} files had issues")
